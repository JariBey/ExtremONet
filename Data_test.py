...
def test_EON(ttr, utr, ytr, tte, ute, yte, ntr):
    s1_best, s2_best = 5, 0.1
    randinds = np.random.choice(ttr.shape[0], ntr, replace=False)
    ttr, ytr, utr = ttr[randinds], ytr[randinds], utr[randinds]
    trunk = ExtremeLearning(1, 1000, c=1, s=s1_best, acfunc=nn.Tanh(),
                            norm=[np.mean(ttr), np.std(ttr)], device=device).to(device)
    branch = ExtremeLearning(utr.shape[1], 1000, c=3, s=s2_best, acfunc=nn.Tanh(),
...
def testloop(ttr, utr, ytr, tte, ute, yte, repeats, datas):
    ...
    for data in datas:
        ...
        for _ in range(repeats):
            res = test_EON(ttr, utr, ytr, tte, ute, yte, data)
...
nrepeat = 30
widths = np.linspace(200, 10000, 20, dtype=int)
...
eth_ODE_param_std, evh_ODE_param_std, ett_ODE_param_std, ept_ODE_param_std = testloop(
    ...
    ODE_param_tte, ODE_param_ute, ODE_param_yte, nrepeat, widths)
...
eth_ODE_GRF_std, evh_ODE_GRF_std, ett_ODE_GRF_std, ept_ODE_GRF_std = testloop(
    ...
    ODE_GRF_tte, ODE_GRF_ute, ODE_GRF_yte, nrepeat, widths)
...
eth_PDE_param_std, evh_PDE_param_std, ett_PDE_param_std, ept_PDE_param_std = testloop(
    ...
    PDE_param_tte, PDE_param_ute, PDE_param_yte, nrepeat, widths)
...
eth_PDE_GRF_std, evh_PDE_GRF_std, ett_PDE_GRF_std, ept_PDE_GRF_std = testloop(
    ...
    PDE_GRF_tte, PDE_GRF_ute, PDE_GRF_yte, nrepeat, widths)
...
ab_shades = generate_shades(base_colors["ab"], num_shades=len(widths))
linestyles = {"a": "-", "b": "-."}
...
for (key, ax), (eth, evh) in zip(axs.items(), data):
    ...
    ax.plot(widths, eth[0], color="blue", linestyle=linestyles["a"], label="a")
    ax.fill_between(widths, np.array(eth[0]) - np.array(eth[1]), 
                    np.array(eth[0]) + np.array(eth[1]), color="blue", alpha=0.2)
    ax.plot(widths, evh[0], color="black", linestyle=linestyles["b"], label="b")
    ax.fill_between(widths, np.array(evh[0]) - np.array(evh[1]), 
...
plt.savefig("Data_comp.svg", dpi=300, bbox_inches="tight", pad_inches=0.05)
...
bar_width = (widths[1] - widths[0]) * 0.8
fig, axs = plt.subplots(1, 2, figsize=(width * 0.8, width * 0.3))

# Train Time Plot
axs[0].bar(widths, mean_ttE, yerr=np.std(mean_ttE, axis=0), width=bar_width, capsize=2, 
...
axs[1].bar(widths, mean_ptE, yerr=np.std(mean_ptE, axis=0), width=bar_width, capsize=2, 
           label="EON", alpha=0.7, color="blue")
...
plt.savefig("Data_time_comp.svg", dpi=300, bbox_inches="tight", pad_inches=0.05)
plt.show()
