...
def test_EON(ttr, utr, ytr, tte, ute, yte, width):
    ...
    trunk = ExtremeLearning(1, width, c=1, s=s1_best, acfunc=nn.Tanh(),
                            norm=[np.mean(ttr), np.std(ttr)], device=device).to(device)
    branch = ExtremeLearning(utr.shape[1], width, c=3, s=s2_best, acfunc=nn.Tanh(),
                             norm=[np.mean(utr), np.std(utr)], device=device).to(device)
    ...
    start_time = time.time()
    _ = EON.predict(tte, ute)
    predtime = time.time() - start_time
    del trunk, branch, EON
    torch.cuda.empty_cache()
    return trhist[np.argmin(valhist)], np.min(valhist), trtime, predtime/tte.shape[0]

def testloop(ttr, utr, ytr, tte, ute, yte, repeats, datas):
    ...
    for width in datas:
        ...
        for _ in range(repeats):
            res = test_EON(ttr, utr, ytr, tte, ute, yte, width)
...
nrepeat = 30
datas = np.linspace(10, 1000, 10, dtype=int)
...
eth_ODE_param_std, evh_ODE_param_std, ett_ODE_param_std, ept_ODE_param_std = testloop(
    ...
    ODE_param_tte, ODE_param_ute, ODE_param_yte, nrepeat, datas)
...
eth_ODE_GRF_std, evh_ODE_GRF_std, ett_ODE_GRF_std, ept_ODE_GRF_std = testloop(
    ...
    ODE_GRF_tte, ODE_GRF_ute, ODE_GRF_yte, nrepeat, datas)
...
eth_PDE_param_std, evh_PDE_param_std, ett_PDE_param_std, ept_PDE_param_std = testloop(
    ...
    PDE_param_tte, PDE_param_ute, PDE_param_yte, nrepeat, datas)
...
eth_PDE_GRF_std, evh_PDE_GRF_std, ett_PDE_GRF_std, ept_PDE_GRF_std = testloop(
    ...
    PDE_GRF_tte, PDE_GRF_ute, PDE_GRF_yte, nrepeat, datas)
...
ab_shades = generate_shades(base_colors["ab"], num_shades=len(datas))
linestyles = {"a": "-", "b": "-."}
...
for (key, ax), (eth, evh) in zip(axs.items(), data):
    ...
    ax.plot(datas, eth[0], color="blue", linestyle=linestyles["a"], label="a",linewidth=1)
    ax.fill_between(datas, np.array(eth[0]) - np.array(eth[1]), 
                    np.array(eth[0]) + np.array(eth[1]), color="blue", alpha=0.2,linewidth=1)
    ax.plot(datas, evh[0], color="black", linestyle=linestyles["b"], label="b",linewidth=1)
    ax.fill_between(datas, np.array(evh[0]) - np.array(evh[1]), 
                    np.array(evh[0]) + np.array(evh[1]), color="black", alpha=0.2,linewidth=1)
    ax.set_title( f"{key})")
    ax.grid()

plt.tight_layout()
fig.subplots_adjust(left=0.1, right=0.9, top=0.9, bottom=0.1, wspace=0.3, hspace=0.5)
plt.savefig("width_comp.svg", dpi=300, bbox_inches="tight", pad_inches=0.05)
...
bar_width = (datas[1] - datas[0]) * 0.8
fig, axs = plt.subplots(1, 2, figsize=(width * 0.8, width * 0.3))

# Train Time Plot
axs[0].bar(datas, mean_ttE, yerr=np.std(mean_ttE, axis=0), width=bar_width, capsize=2, 
           label="EON", alpha=0.7, color="blue",linewidth=1)
...
axs[1].bar(datas, mean_ptE, yerr=np.std(mean_ptE, axis=0), width=bar_width, capsize=2, 
           label="EON", alpha=0.7, color="blue",linewidth=1)
...
plt.savefig("width_time_comp.svg", dpi=300, bbox_inches="tight", pad_inches=0.05)
plt.show()
