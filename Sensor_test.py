...
def test_EON(ttr, utr, ytr, tte, ute, yte, sensors):
    ...
    if sensors == 0:
        ...
        randinds = np.random.choice(range(utr.shape[1]), sensors, replace=False)
        utr, ute = utr[:, randinds], ute[:, randinds]
        # utr, ute = utr[:,:sensors], ute[:, :sensors]
        trunk_norm = [np.mean(ttr), np.std(ttr)]
    ...
    branch = ExtremeLearning(utr.shape[1], 1000, c=3, s=s2_best, acfunc=nn.Tanh(), norm=branch_norm, device=device).to(device)
...
plt.savefig("Sensors_comp.svg", dpi=300, bbox_inches="tight", pad_inches=0.05)
...
plt.savefig("Sensor_time_comp.svg", dpi=300, bbox_inches="tight", pad_inches=0.05)
plt.show()
